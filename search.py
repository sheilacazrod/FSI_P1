...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print("Nodos generados: ", generados, ", Nodos visitados: ", visitados, ", coste: ", node.path_cost)
            return node
        if node.state not in closed:
            closed[node.state] = True
            expansion=node.expand(problem)
            fringe.extend(expansion)
            generados += len(expansion)
    return None
...
def branch_and_bound_search_sub(problem):
   return graph_search(problem,PriorityQueueWithSub(problem))
